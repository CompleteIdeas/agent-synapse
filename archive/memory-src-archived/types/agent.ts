...
export interface AgentConfig {
  ...
  // Reinforcement-on-duplicate (v0.5.0)
  reinforceNoveltyThreshold: number;   // Below this novelty → candidate for reinforcement
  reinforceBM25Threshold: number;      // Above this BM25 match → confirm reinforcement
  reinforceConfidenceBoost: number;    // How much to boost confidence on reinforce
  reinforceSalienceBoost: number;      // How much to boost salience on reinforce
}
...
export const DEFAULT_AGENT_CONFIG: AgentConfig = {
  ...
  reinforceNoveltyThreshold: 0.3,
  reinforceBM25Threshold: 0.85,
  reinforceConfidenceBoost: 0.05,
  reinforceSalienceBoost: 0.02,
};
