...
export class EngramStore {
  ...
  private init(): void {
    ...
    // Migration: v0.5.0 — reinforcement count
    try {
      this.db.prepare('SELECT reinforcement_count FROM engrams LIMIT 0').get();
    } catch {
      this.db.exec('ALTER TABLE engrams ADD COLUMN reinforcement_count INTEGER NOT NULL DEFAULT 0');
    }

    // Migration: v0.5.0 — workspace scope
    try {
      this.db.prepare('SELECT scope FROM engrams LIMIT 0').get();
    } catch {
      this.db.exec("ALTER TABLE engrams ADD COLUMN scope TEXT NOT NULL DEFAULT 'agent'");
      this.db.exec('ALTER TABLE engrams ADD COLUMN workspace TEXT');
      this.db.exec('CREATE INDEX IF NOT EXISTS idx_engrams_workspace ON engrams(workspace, scope, stage)');
    }

    // Migration: v0.5.0 — source provenance
    try {
      this.db.prepare('SELECT source FROM engrams LIMIT 0').get();
    } catch {
      this.db.exec("ALTER TABLE engrams ADD COLUMN source TEXT NOT NULL DEFAULT '{}'");
    }
  ...
  createEngram(input: EngramCreate): Engram {
    ...
    this.db.prepare(`
      INSERT INTO engrams (id, agent_id, concept, content, embedding, confidence, salience,
        ...
        ttl, task_status, task_priority, blocked_by, scope, workspace, source)
      VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      JSON.stringify(input.source ?? { agent: input.agentId }),
    );
  ...
  // --- v0.5.0: Reinforcement ---

  reinforceEngram(id: string, opts: {
    confidenceBoost: number;
    salienceBoost: number;
    reasonCode: string;
  }): Engram | null {
    const engram = this.getEngram(id);
    if (!engram) return null;
    const now = new Date().toISOString();
    const newReasonCodes = [...engram.reasonCodes, opts.reasonCode];
    this.db.prepare(`
      UPDATE engrams SET
        ...
        reason_codes = ?
      WHERE id = ?
    `).run(opts.confidenceBoost, opts.salienceBoost, now, JSON.stringify(newReasonCodes), id);
    return this.getEngram(id);
  }

  // --- v0.5.0: Workspace scope ---

  getWorkspaceEngrams(workspace: string, stage?: EngramStage): Engram[] {
    let sql = "SELECT * FROM engrams WHERE workspace = ? AND scope = 'workspace' AND retracted = 0";
  ...
  updateContent(id: string, content: string): void {
    this.db.prepare('UPDATE engrams SET content = ?, last_accessed = ? WHERE id = ?')
      .run(content, new Date().toISOString(), id);
  }

  // --- v0.5.0: Profiles ---

  getProfiles(agentId: string): Engram[] {
    return (this.db.prepare(
      "SELECT * FROM engrams WHERE agent_id = ? AND stage = 'profile' AND retracted = 0 ORDER BY last_accessed DESC"
    ).all(agentId) as any[]).map(r => this.rowToEngram(r));
  }
   ...
   */
  searchBM25WithRank(
    agentId: string, query: string, limit: number = 10,
    opts?: { includeWorkspace?: string }
  ): { engram: Engram; bm25Score: number }[] {
    ...
    try {
      ...
      if (opts?.includeWorkspace) {
        // Cross-scope search: agent's own memories + workspace-scoped memories
        rows = this.db.prepare(`
          SELECT e.*, rank FROM engrams e
          JOIN engrams_fts ON e.rowid = engrams_fts.rowid
          WHERE engrams_fts MATCH ? AND e.retracted = 0
            AND (e.agent_id = ? OR (e.workspace = ? AND e.scope = 'workspace'))
          ORDER BY rank
          LIMIT ?
        `).all(sanitized, agentId, opts.includeWorkspace, limit) as any[];
      } else {
        rows = this.db.prepare(`
          SELECT e.*, rank FROM engrams e
          JOIN engrams_fts ON e.rowid = engrams_fts.rowid
