...
export class ActivationEngine {
  ...
  async activate(query: ActivationQuery): Promise<ActivationResult[]> {
    ...
    // Keyword stripping removes stopwords and question words that dilute BM25 on conversational data.
    ...
    // Cognitive inspiration: selective attention + hippocampal mismatch detection.
    // Requires at least 2 of 3 retrieval channels to agree the query is in-domain.
    // This replaces the weaker single-score abstention gate.
    if (rerankPool.length >= 3) {
      ...
      // Channel thresholds — calibrated for in-domain relevance
      ...
      // Margin requirement: top reranker result must stand out from the pack
      ...
      // Semantic drift check: raw cosine distribution
    ...
    // Legacy abstention gate (when explicitly requested via abstentionThreshold)
