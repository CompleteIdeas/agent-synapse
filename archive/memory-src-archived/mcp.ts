 ...
 *   memory_recall      — activate memories by context (cognitive retrieval, workspace-aware)
 ...
 *   memory_profile_set — create/update a workspace profile (stable context)
 *
...
import { evaluateSalience, computeNoveltyWithMatch } from './core/salience.js';
import { deriveWorkspace } from './core/workspace.js';
import type { EngramSource } from './types/engram.js';
...
The concept should be a short label (3-8 words). The content should be the full detail.`,
  {
    ...
    scope: z.enum(['agent', 'workspace']).optional().default('agent')
      .describe("'agent' (private, default) or 'workspace' (visible to all agents in same workspace)"),
    source: z.object({
      task: z.string().optional(),
      file: z.string().optional(),
      context: z.string().optional(),
    }).optional().describe('Where this memory came from (agent is auto-populated)'),
  },
  async (params) => {
    const workspace = deriveWorkspace(AGENT_ID);

    // Check novelty — is this new information or a duplicate?
    const noveltyResult = computeNoveltyWithMatch(store, AGENT_ID, params.concept, params.content, workspace);
    const novelty = noveltyResult.novelty;

    // --- Reinforce-on-Duplicate check ---
    // Tightened thresholds (0.5.4 fix): require near-exact match to reinforce,
    // and never early-return — always continue to create the new memory if
    // content meaningfully differs from the matched engram.
    ...
    if (novelty < 0.3
      ...
      if (matchedEngram) {
        // Semantic content check: only reinforce if content is truly duplicative
        ...
        if (contentOverlap > 0.6) {
          ...
          const result = store.reinforceEngram(noveltyResult.matchedEngramId, {
            confidenceBoost: DEFAULT_AGENT_CONFIG.reinforceConfidenceBoost,
            salienceBoost: DEFAULT_AGENT_CONFIG.reinforceSalienceBoost,
            reasonCode: `reinforced:${new Date().toISOString()}:by:${AGENT_ID}`,
          });
          if (result) {
            try { store.updateAutoCheckpointWrite(AGENT_ID, result.id); } catch { /* non-fatal */ }
            log(AGENT_ID, 'write:reinforce', `"${params.concept}" → reinforced "${result.concept}" (conf=${result.confidence.toFixed(2)}, n=${result.reinforcementCount}, overlap=${contentOverlap.toFixed(2)})`);
            return {
              content: [{
                type: 'text' as const,
                text: `Reinforced existing memory "${result.concept}" (confidence now ${result.confidence.toFixed(2)}, reinforced ${result.reinforcementCount} times)`,
              }],
            };
          }
        } else {
          // Partial match — reinforce existing but ALSO create the new memory (don't early return)
          store.reinforceEngram(noveltyResult.matchedEngramId, {
            confidenceBoost: DEFAULT_AGENT_CONFIG.reinforceConfidenceBoost * 0.5,
            salienceBoost: DEFAULT_AGENT_CONFIG.reinforceSalienceBoost * 0.5,
            reasonCode: `partial-reinforce:${new Date().toISOString()}:by:${AGENT_ID}`,
          });
          reinforced = true;
          log(AGENT_ID, 'write:partial-reinforce', `"${params.concept}" partially matched "${matchedEngram.concept}" (overlap=${contentOverlap.toFixed(2)}), creating new memory`);
        }
      }
    }
    ...
    // v0.5.4: No longer discard — store everything, use salience for ranking instead.
    // Low-salience memories get low confidence (0.25) so they rank below high-salience
    // memories in retrieval, but remain available for recall when needed.
    // This fixes LoCoMo (was discarding 57% of answer-bearing turns).
    ...
    const confidencePrior = isLowSalience
      ? 0.25  // Low-salience: stored but ranked below high-salience in retrieval
    ...
    const source: EngramSource = { agent: AGENT_ID, ...params.source };
    ...
    const engram = store.createEngram({
      ...
      scope: params.scope as 'agent' | 'workspace',
      workspace: params.scope === 'workspace' ? (workspace ?? undefined) : undefined,
      source,
    });
...
Returns the most relevant memories ranked by text relevance, temporal recency, and associative strength.`,
  ...
  async (params) => {
    ...
    const lines = results.map((r, i) => {
      const srcInfo = r.engram.source?.agent && r.engram.source.agent !== AGENT_ID
        ? ` [from: ${r.engram.source.agent}]` : '';
      return `${i + 1}. **${r.engram.concept}** (${r.score.toFixed(3)})${srcInfo}: ${r.engram.content}`;
    });
...
Use this at the start of every session or after compaction to pick up where you left off.`,
  ...
  async () => {
    ...
    // v0.5.0: Inject workspace profiles (task-conditional)
    const profiles = store.getProfiles(AGENT_ID);
    if (profiles.length > 0) {
      let relevantProfiles = profiles;
      if (recallContext && profiles.length > 3) {
        const contextTokens = new Set(recallContext.toLowerCase().split(/\s+/).filter(w => w.length > 3));
        relevantProfiles = profiles.filter(p => {
          const profileTokens = `${p.concept} ${p.content}`.toLowerCase().split(/\s+/).filter(w => w.length > 3);
          return profileTokens.some(t => contextTokens.has(t));
        });
        if (relevantProfiles.length === 0) relevantProfiles = profiles.slice(0, 3);
      } else {
        relevantProfiles = profiles.slice(0, 5);
      }
      if (relevantProfiles.length > 0) {
        parts.push(`\n**Workspace profiles (${relevantProfiles.length}):**`);
        for (const p of relevantProfiles) {
          parts.push(`- **${p.concept}**: ${p.content.slice(0, 200)}${p.content.length > 200 ? '...' : ''}`);
        }
      }
    }
...
server.tool(
  'memory_profile_set',
  `Create or update a workspace profile — stable context about your project or workspace.

Profiles are high-priority, persist indefinitely, and are injected into memory_restore results.
Use for conventions, patterns, preferences, or facts that apply broadly across sessions.

Examples: "coding conventions", "project architecture", "user preferences", "team structure"`,
  {
    concept: z.string().describe('Profile topic (e.g., "coding conventions", "project architecture")'),
    content: z.string().describe('The profile content — stable facts and conventions'),
    tags: z.array(z.string()).optional().describe('Optional tags for categorization'),
  },
  async (params) => {
    // Check if a profile with this concept already exists
    const existing = store.getProfiles(AGENT_ID).find(
      p => p.concept.toLowerCase() === params.concept.toLowerCase()
    );

    if (existing) {
      store.updateContent(existing.id, params.content);
      store.touchEngram(existing.id);
      log(AGENT_ID, 'profile:update', `"${params.concept}" updated`);
      return {
        content: [{ type: 'text' as const, text: `Profile updated: "${params.concept}"` }],
      };
    }
    ...
    const engram = store.createEngram({
      ...
      concept: params.concept,
      content: params.content,
      tags: [...(params.tags ?? []), 'profile'],
      salience: 0.95,
      confidence: 0.9,
      reasonCodes: ['profile-created'],
    });
    store.updateStage(engram.id, 'profile');

    embed(`${params.concept} ${params.content}`).then(vec => {
      store.updateEmbedding(engram.id, vec);
    }).catch(() => {});

    log(AGENT_ID, 'profile:create', `"${params.concept}" id=${engram.id}`);
    return {
      content: [{ type: 'text' as const, text: `Profile created: "${params.concept}"` }],
    };
  }
);
