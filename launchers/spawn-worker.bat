@echo off
:: Spawn a worker for a specific task. Called by the orchestrator.
...
echo claude --dangerously-skip-permissions --agent worker --append-system-prompt "YOUR IDENTITY: You are %WORKER_NAME%. Display [%WORKER_NAME%] at the start of every response. WORKER_NAME=%WORKER_NAME%. PROJECT DIRECTORY: %PROJECT_DIR%. You were spawned by the orchestrator for a SPECIFIC TASK. Complete it, report results via POST /findings or POST /assign update, then checkin as idle and wait for further instructions." "%TASK%" >> "%TMPSCRIPT%"

:: Launch in a new Windows Terminal tab
wt -w "AgentSynapse-Personal" new-tab --title "%WORKER_NAME% [task]" cmd /k "%TMPSCRIPT%"
