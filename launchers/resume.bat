...
if not "%~1"=="" (
    curl -s -X POST http://127.0.0.1:8400/command -H "Content-Type: application/json" -d "{\"command\":\"RESUME\",\"workspace\":\"%~1\"}" >nul
    echo  RESUME issued for workspace: %~1
) else (
    curl -s -X POST http://127.0.0.1:8400/command -H "Content-Type: application/json" -d "{\"command\":\"RESUME\"}" >nul
