@echo off
:: Launches hive for WORK workspace: orchestrator + dev-lead + 3 workers
...
where wt >nul 2>&1
if %errorlevel% equ 0 (
    wt new-tab --title "Orchestrator [work]" cmd /k "%LAUNCHER_DIR%start-worker.bat orchestrator %PROJECT_DIR%" ; ^
    ...
    start "Orchestrator [work]" cmd /k "%LAUNCHER_DIR%start-worker.bat orchestrator %PROJECT_DIR%"
...
echo    Orchestrator + Dev-Lead + Worker-A + Worker-B + Worker-C
echo  Project: %PROJECT_DIR%
echo.