...
export function createPushAdapter(userConfig: Partial<PushConfig> = {}) {
  ...
  function loadState(): number | null {
    try {
      ...
      const v = (JSON.parse(raw) as { lastEventId?: number }).lastEventId;
  ...
  function saveState(): void {
    try { writeFileSync(STATE_PATH, JSON.stringify({ lastEventId }), 'utf8'); } catch { /* best effort */ }
  }

  /** Page to the end of the assignment-event feed (NO pushes) and return the max id seen. */
  async function fetchMaxEventId(fromId: number): Promise<number> {
    let max = fromId;
   ...
   */
  async function handleAssignmentCreated(agentId: string, detail: string, sessions?: ChannelSession[]): Promise<PushResult> {
    ...
    try {
      ...
      if (!session) return result; // no channel — agent is polling, no push needed
      ...
      if (!channelRes.ok) { result.error = `channel push failed: ${channelRes.status}`; return result; }
   ...
   */
  async function initThenPoll(): Promise<void> {
    try {
