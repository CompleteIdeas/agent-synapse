...
async function main() {
    ...
    const server = new Server({ name: 'awm', version: '0.1.0' }, {
        ...
        instructions: `AWM coordination channel for ${WORKER_NAME}. ` +
    ...
    (async () => {
        for (let attempt = 0; attempt < 12; attempt++) {
            try {
                ...
                if (!nextRes.ok)
                    throw new Error(`/next returned ${nextRes.status}`);
                const nextData = await nextRes.json();
                if (!nextData.agentId)
                    throw new Error('/next missing agentId');
                ...
                if (!regRes.ok)
                    throw new Error(`/channel/register returned ${regRes.status}`);
                process.stderr.write(`[awm-channel] Registered: ${WORKER_NAME} (${nextData.agentId}) → ${channelUrl}\n`);
                return; // success
            }
            catch (err) {
                process.stderr.write(`[awm-channel] Registration attempt ${attempt + 1}/12: ${err}\n`);
