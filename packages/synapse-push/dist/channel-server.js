    ...
    (async () => {
        for (let attempt = 0; attempt < 12; attempt++) {
            try {
                ...
                if (!res.ok)
                    throw new Error(`/checkin returned ${res.status}`);
                const data = await res.json();
                if (!data.agentId)
                    throw new Error('/checkin missing agentId');
                process.stderr.write(`[awm-channel] Registered: ${WORKER_NAME} (${data.agentId}) → ${channelUrl} [${data.action}]\n`);
                return; // success
            }
            catch (err) {
