...
- **AWM** ([AgentWorkingMemory](https://github.com/CompleteIdeas/agent-working-memory), v0.7.16) — Cognitive memory layer with activation-based retrieval, salience filtering, Hebbian learning. Agents remember across sessions. Also serves as the coordination backend when `AWM_COORDINATION=true`. Recall latency on a 17K-engram corpus is ~300ms typical, 400-700ms median (down from 11-23s pre-0.7.6) so agents can recall freely. LongMemEval single-session-user: 68% with gpt-4o-mini, 80% with gpt-5-mini (up from the 0.7.1 baseline of 40-50%).
...
- **2026-05-10 — AWM 0.7.16: write-quality instruction template + LongMemEval re-run.** The `awm setup --global` template now teaches two behaviors that AWM was always designed for but had been implicit: *Writing for recall* (lead with the rule, pick the most specific topic, include 2+ retrievable identifiers, write in the vocabulary of the future query) and *Recall strategy* (when one query returns nothing, reformulate — recall is ~300ms, two-three reformulations cost less than one filesystem search). No retriever change. Re-ran LongMemEval (single-session-user, 50 questions, same adapter as the 0.7.1 baseline): **68% with gpt-4o-mini (up from 40-50%)**, 80% with gpt-5-mini. Recall latency 0.12s avg vs prior 7-11s.
